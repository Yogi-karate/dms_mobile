...
import static com.dealermanagmentsystem.constants.Constants.INSURANCE_LOST_REASON;
...
import static com.dealermanagmentsystem.constants.ConstantsUrl.GET_INSURANCE_LOST_REASON;
...
public class InsuranceLeadsPresenter implements IInsuranceLeadPresenter {

    IInsuranceLeadView view;

    public InsuranceLeadsPresenter(IInsuranceLeadView insuranceLeadView) {
        view = insuranceLeadView;
    }
    ...
    public void getInsuranceLeads(Activity activity, String strState, boolean isCheckedMyLeads) {
        ...
        String url;
        if (isCheckedMyLeads) {
            url =SERVICE_LEADS + "?state=" + strState+"&callType=Insurance" + "&userId=" + DMSPreference.getInt(KEY_USER_ID);
        } else {
            url = SERVICE_LEADS + "?state=" + strState+"&callType=Insurance";
        }
        ...
        AsyncTaskConnection asyncTaskConnection = new AsyncTaskConnection(url, activity, GET, new IConnectionListener() {
            ...
            public void onSuccess(Result result) {
                ...
                try {
                    ...
                    view.onSuccessInsuranceLeads(enquiryResponse);
    ...
    public void getLostReason(Activity activity, final String id) {
        AsyncTaskConnection asyncTaskConnection = new AsyncTaskConnection(GET_INSURANCE_LOST_REASON, activity, GET, new IConnectionListener() {
            @Override
            public void onSuccess(Result result) {
                JSONObject jsonObject;
                try {
    ...
    public void markLost(Activity activity, String id, String isAction, int lostReason) {
        ...
        try {
            ...
            postDataParams.put(INSURANCE_LOST_REASON, lostReason);
